...
@Component({
  ...
  host: { class: 'relative flex-shrink-0 flex h-full' },
})
export class AppRightRailComponent {
  ...
  toggleTrigger(index: number): void {
    this.triggers.update((list) => list.map((t, i) => (i === index ? { ...t, on: !t.on } : t)));
  }
}
